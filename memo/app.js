...
const i18nDict = {
  ko: {
    ...
    "label-wordwrap": "줄바꿈",
    "history-title": "리비전 히스토리",
    "history-empty": "저장된 리비전이 없습니다.",
    "history-compare": "차이 비교",
    "history-preview": "원문 보기",
    "history-filter-changed": "변경 줄만",
    "history-filter-all": "전체 보기",
    "history-delete": "이 리비전 삭제",
    "history-restore": "이 리비전 복원",
    "history-selected-none": "선택된 리비전이 없습니다.",
    "history-meta": "길이 {chars}자 · 줄 수 {lines}",
    "history-current-label": "현재 문서",
    "history-revision-label": "선택한 리비전",
    "history-diff-empty": "변경된 줄이 없습니다.",
    "history-delete-confirm": "이 리비전을 삭제하시겠습니까?",
    "msg-history-deleted": "리비전이 삭제됨",
    "msg-history-restored": "리비전이 현재 탭에 복원됨",
    "msg-history-empty": "복원할 리비전이 없습니다",
    "prompt-rename":
      "탭 이름 변경 (확장자 포함 시 구문 색상이 자동 감지됩니다):",
  ...
  en: {
    ...
    "history-empty": "No saved revisions for this tab.",
    "history-compare": "Compare Diff",
    "history-preview": "Preview",
    "history-filter-changed": "Changed Only",
    "history-filter-all": "Show All",
    "history-delete": "Delete Revision",
    "history-restore": "Restore This Revision",
    "history-selected-none": "No revision selected.",
    "history-meta": "{chars} chars · {lines} lines",
    "history-current-label": "Current Document",
    "history-revision-label": "Selected Revision",
    "history-diff-empty": "No changed lines.",
    "history-delete-confirm": "Delete this revision?",
    "msg-history-deleted": "Revision deleted",
    "msg-history-restored": "Revision restored into current tab",
    "msg-history-empty": "No revision available to restore",
    "prompt-rename":
      "Rename tab (include extension for auto syntax detection):",
  ...
  ja: {
    ...
    "label-wordwrap": "折返し",
    "history-title": "リビジョン履歴",
    "history-empty": "このタブに保存されたリビジョンはありません。",
    "history-compare": "差分比較",
    "history-preview": "本文表示",
    "history-filter-changed": "変更行のみ",
    "history-filter-all": "全体表示",
    "history-delete": "このリビジョンを削除",
    "history-restore": "このリビジョンを復元",
    "history-selected-none": "選択されたリビジョンはありません。",
    "history-meta": "{chars}文字 ・ {lines}行",
    "history-current-label": "現在の文書",
    "history-revision-label": "選択したリビジョン",
    "history-diff-empty": "変更された行はありません。",
    "history-delete-confirm": "このリビジョンを削除しますか？",
    "msg-history-deleted": "リビジョンを削除しました",
    "msg-history-restored": "現在のタブにリビジョンを復元しました",
    "msg-history-empty": "復元できるリビジョンがありません",
  ...
  "zh-TW": {
    ...
    "label-wordwrap": "換行",
    "history-title": "修訂紀錄",
    "history-empty": "這個分頁尚無已儲存修訂。",
    "history-compare": "差異比較",
    "history-preview": "原文檢視",
    "history-filter-changed": "僅看變更行",
    "history-filter-all": "顯示全部",
    "history-delete": "刪除此修訂",
    "history-restore": "還原此修訂",
    "history-selected-none": "尚未選取修訂。",
    "history-meta": "{chars} 字 · {lines} 行",
    "history-current-label": "目前文件",
    "history-revision-label": "所選修訂",
    "history-diff-empty": "沒有變更的行。",
    "history-delete-confirm": "要刪除此修訂嗎？",
    "msg-history-deleted": "修訂已刪除",
    "msg-history-restored": "修訂已還原到目前分頁",
    "msg-history-empty": "沒有可還原的修訂",
  ...
  "zh-CN": {
    ...
    "label-wordwrap": "换行",
    "history-title": "修订历史",
    "history-empty": "这个标签页还没有已保存修订。",
    "history-compare": "差异比较",
    "history-preview": "原文查看",
    "history-filter-changed": "仅看变更行",
    "history-filter-all": "显示全部",
    "history-delete": "删除此修订",
    "history-restore": "恢复此修订",
    "history-selected-none": "尚未选择修订。",
    "history-meta": "{chars} 字 · {lines} 行",
    "history-current-label": "当前文档",
    "history-revision-label": "所选修订",
    "history-diff-empty": "没有变更的行。",
    "history-delete-confirm": "要删除此修订吗？",
    "msg-history-deleted": "修订已删除",
    "msg-history-restored": "修订已恢复到当前标签页",
    "msg-history-empty": "没有可恢复的修订",
...
async function saveToStorage() {
  ...
  try {
    ...
    const savedMsg = i18nDict[appData.uiLang]
      ? i18nDict[appData.uiLang]["status-saved"]
      : "저장됨";
    showStatus(savedMsg);
  } catch (err) {
    console.error("Save to IndexedDB failed", err);
    const errMsg = i18nDict[appData.uiLang]
      ? i18nDict[appData.uiLang]["status-error"]
      : "저장 오류";
...
function applyLanguage(langCode) {
  ...
  for (const [id, text] of Object.entries(dict)) {
    ...
    if (el) {
      ...
      if (el.tagName === "SPAN" && el.id === "search-title")
        el.textContent = text;
      if (el.tagName === "BUTTON" && el.id === "btn-replace")
        el.textContent = text;
      if (el.tagName === "BUTTON" && el.id === "btn-replace-all")
        el.textContent = text;
      if (el.tagName === "SPAN" && el.id === "search-status")
        el.textContent = "";
    }
  }
  ...
  if (searchInput)
    searchInput.setAttribute("placeholder", dict["search-placeholder"]);
  if (replaceInput)
    replaceInput.setAttribute("placeholder", dict["replace-placeholder"]);

  // Status Default
  const statusMsg = document.getElementById("status-msg");
  if (
    statusMsg &&
    statusMsg.textContent === i18nDict[appData.uiLang]["status-ready"]
  ) {
  ...
  if (historyTitle) historyTitle.textContent = dict["history-title"];
  const historyEmpty = document.getElementById("history-empty");
  if (historyEmpty) historyEmpty.textContent = dict["history-empty"];
  const historyCompare = document.getElementById("btn-history-compare");
  if (historyCompare)
    historyCompare.textContent =
      revisionHistoryState.viewMode === "diff"
        ? dict["history-preview"]
        : dict["history-compare"];
  const historyFilter = document.getElementById("btn-history-filter");
  if (historyFilter)
    historyFilter.textContent = revisionHistoryState.showChangedOnly
      ? dict["history-filter-all"]
      : dict["history-filter-changed"];
  const historyDelete = document.getElementById("btn-history-delete");
  if (historyDelete) historyDelete.textContent = dict["history-delete"];
  const historyRestore = document.getElementById("btn-history-restore");
  if (historyRestore) historyRestore.textContent = dict["history-restore"];
...
async function autoDetectSyntax(filename) {
  ...
  if (activeTab) {
    ...
    let optionExists = Array.from(langSelect.options).some(
      (opt) => opt.value === mime,
    );
    if (!optionExists) {
      ...
      if (update.docChanged) {
        ...
        if (activeTab && activeTab.content !== cm.state.doc.toString()) {
          ...
          const ui = document.querySelector(
            `.tab[data-id="${activeTab.id}"] .tab-title`,
          );
...
function renderTabs() {
  ...
  appData.tabs.forEach((tab, index) => {
    ...
    let displayTitle =
      tab.title ||
      (newTabTitles[appData.uiLang] || "Untitled") + " " + (index + 1);
...
function loadActiveTabContent() {
  ...
  if (activeTab) {
    ...
    const lgDesc = languages.find(
      (l) =>
        l.name === lName ||
        l.name.toLowerCase() === lName.toLowerCase() ||
        (l.alias && l.alias.includes(lName.toLowerCase())),
    );
    if (lgDesc) {
      lgDesc
        .load()
        .then((ls) => cm.dispatch({ effects: languageConf.reconfigure(ls) }));
    } else {
      cm.dispatch({ effects: languageConf.reconfigure([]) });
    }
    cm.dispatch({
      effects: readOnlyConf.reconfigure(
        EditorState.readOnly.of(!!activeTab.readonly),
      ),
    });
  ...
  const currentTabUI = document.querySelector(
    `.tab[data-id="${appData.activeTabId}"] .tab-title`,
  );
...
function getLocaleForFormatting() {
  ...
  return localeMap[appData.uiLang] || "ko-KR";
}
...
function formatHistoryMeta(content = "") {
  const text = content;
  const chars = text.length;
  const lines = text === "" ? 0 : text.split("\n").length;
  return t("history-meta", "{chars} chars · {lines} lines")
    .replace("{chars}", chars)
    .replace("{lines}", lines);
}

async function getSnapshotsByTabId(tabId) {
  const snapshots = (await localforage.getItem("webmemo_snapshots")) || {};
  return Array.isArray(snapshots[tabId]) ? snapshots[tabId] : [];
}

async function setSnapshotsByTabId(tabId, nextItems) {
  const snapshots = (await localforage.getItem("webmemo_snapshots")) || {};
  snapshots[tabId] = nextItems;
  await localforage.setItem("webmemo_snapshots", snapshots);
}
...
function getDiffRowType(left, right) {
...
function buildDiffMatrix(leftLines, rightLines) {
  const matrix = Array.from({ length: leftLines.length + 1 }, () =>
    new Array(rightLines.length + 1).fill(0),
  );
  for (let leftIndex = leftLines.length - 1; leftIndex >= 0; leftIndex--) {
    for (
...
function appendRemainingDiffRows(
  rows,
  leftLines,
  rightLines,
  leftIndex,
  ...
  while (nextLeftIndex < leftLines.length) {
    rows.push({ left: leftLines[nextLeftIndex], right: null, type: "removed" });
    nextLeftIndex++;
  }

  while (nextRightIndex < rightLines.length) {
    rows.push({ left: null, right: rightLines[nextRightIndex], type: "added" });
    nextRightIndex++;
  }
}

function getDiffLineClass(type, side) {
  if (type === "same") return "history-diff-line-same";
  if (type === "added" && side === "right") return "history-diff-line-added";
  if (type === "removed" && side === "left") return "history-diff-line-removed";
  return "";
}

function buildFallbackDiffRows(leftLines, rightLines) {
  ...
  for (let index = 0; index < rowCount; index++) {
    ...
    rows.push({
      left: left ?? null,
      right: right ?? null,
      type: getDiffRowType(left, right),
    });
  }
  return rows;
}

function buildLineDiffRows(currentContent, revisionContent) {
  const leftLines = currentContent.split("\n");
  const rightLines = revisionContent.split("\n");
  const maxCells = leftLines.length * rightLines.length;

  if (maxCells > 120000) {
    return buildFallbackDiffRows(leftLines, rightLines);
  }

  const dp = buildDiffMatrix(leftLines, rightLines);
  ...
  while (leftIndex < leftLines.length && rightIndex < rightLines.length) {
    ...
    if (dp[leftIndex + 1][rightIndex] >= dp[leftIndex][rightIndex + 1]) {
      rows.push({ left: leftLines[leftIndex], right: null, type: "removed" });
      leftIndex++;
    } else {
      rows.push({ left: null, right: rightLines[rightIndex], type: "added" });
      rightIndex++;
    }
  }

  appendRemainingDiffRows(rows, leftLines, rightLines, leftIndex, rightIndex);

  return rows;
}

function renderDiffPane(title, side, rows) {
  let lineNumber = 0;
  const body = rows
    .map((row) => {
      const value = row[side];
      const lineClass = getDiffLineClass(row.type, side);
      const numberText = value == null ? "" : String(++lineNumber);
      const contentText = value == null ? "" : escapeHtml(value);
      return `<div class="history-diff-line ${lineClass}"><div class="history-diff-line-num">${numberText}</div><div class="history-diff-line-text">${contentText}</div></div>`;
    })
    .join("");

  return `<section class="history-diff-pane"><div class="history-diff-heading">${escapeHtml(title)}</div><div class="history-diff-lines">${body}</div></section>`;
}

function renderRevisionDiff(selected) {
  const diffContainer = document.getElementById("history-diff-content");
  const activeTab = appData.tabs.find((t) => t.id === appData.activeTabId);
  if (!diffContainer || !activeTab || !selected) return;

  const rows = buildLineDiffRows(
    activeTab.content || "",
    selected.content || "",
  );
  const visibleRows = revisionHistoryState.showChangedOnly
    ? rows.filter((row) => row.type !== "same")
    : rows;

  if (visibleRows.length === 0) {
    diffContainer.innerHTML = `<div class="history-diff-empty">${escapeHtml(t("history-diff-empty"))}</div>`;
    return;
  }

  diffContainer.innerHTML = [
    renderDiffPane(t("history-current-label"), "left", visibleRows),
    renderDiffPane(t("history-revision-label"), "right", visibleRows),
  ].join("");
}

function closeRevisionHistoryPanel() {
  const historyModal = document.getElementById("history-modal");
...
function renderRevisionHistoryPanel() {
  const historyList = document.getElementById("history-list");
  const historyEmpty = document.getElementById("history-empty");
  const historySelectedTime = document.getElementById("history-selected-time");
  const historySelectedInfo = document.getElementById("history-selected-info");
  const historyPreviewContent = document.getElementById(
  ...
  const filterButton = document.getElementById("btn-history-filter");
  const deleteButton = document.getElementById("btn-history-delete");
  const restoreButton = document.getElementById("btn-history-restore");
  ...
  const items = revisionHistoryState.items;

  if (items.length === 0) {
    historyEmpty.classList.remove("hidden");
    historySelectedTime.textContent = t("history-selected-none");
    historySelectedInfo.textContent = "";
    historyPreviewContent.textContent = "";
    historyDiffContent.innerHTML = "";
    historyPreviewContent.classList.remove("hidden");
    historyDiffContent.classList.add("hidden");
    compareButton.disabled = true;
    compareButton.classList.remove("active");
    compareButton.textContent = t("history-compare");
    filterButton.disabled = true;
    filterButton.classList.remove("active");
    filterButton.textContent = t("history-filter-changed");
  ...
  const safeIndex = Math.min(
    Math.max(revisionHistoryState.selectedIndex, 0),
    items.length - 1,
  );
  revisionHistoryState.selectedIndex = safeIndex;

  items.forEach((item, index) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "history-item" + (index === safeIndex ? " active" : "");
    button.dataset.index = index;

    const timeEl = document.createElement("div");
    timeEl.className = "history-item-time";
    timeEl.textContent = formatRevisionTimestamp(item.timestamp);

    const metaEl = document.createElement("div");
    metaEl.className = "history-item-meta";
    metaEl.textContent = formatHistoryMeta(item.content);

    const snippetEl = document.createElement("div");
    snippetEl.className = "history-item-snippet";
    snippetEl.textContent = (item.content || "").trim() || " ";
  ...
  historySelectedTime.textContent = formatRevisionTimestamp(selected.timestamp);
  historySelectedInfo.textContent = formatHistoryMeta(selected.content);
  historyPreviewContent.textContent = selected.content || "";
  deleteButton.disabled = false;
  restoreButton.disabled = false;

  if (revisionHistoryState.viewMode === "diff") {
    renderRevisionDiff(selected);
    historyPreviewContent.classList.add("hidden");
    historyDiffContent.classList.remove("hidden");
    compareButton.classList.add("active");
    compareButton.textContent = t("history-preview");
    filterButton.disabled = false;
    filterButton.classList.toggle(
      "active",
      revisionHistoryState.showChangedOnly,
    );
    filterButton.textContent = revisionHistoryState.showChangedOnly
      ? t("history-filter-all")
      : t("history-filter-changed");
  } else {
    historyDiffContent.innerHTML = "";
    historyPreviewContent.classList.remove("hidden");
    historyDiffContent.classList.add("hidden");
...
async function openRevisionHistoryPanel() {
  const activeTab = appData.tabs.find((t) => t.id === appData.activeTabId);
  if (!activeTab) return;

  const snapshots = await getSnapshotsByTabId(activeTab.id);
  revisionHistoryState = {
    items: snapshots
      .slice()
      .reverse()
      .map((snapshot, index) => ({
        ...snapshot,
        sourceIndex: snapshots.length - 1 - index,
      })),
    selectedIndex:
      snapshots.length > 0
        ? Math.min(
        ...
        : -1,
    viewMode: revisionHistoryState.viewMode || "preview",
    showChangedOnly: revisionHistoryState.showChangedOnly || false,
  };
...
async function restoreSelectedRevision() {
  const activeTab = appData.tabs.find((t) => t.id === appData.activeTabId);
  const selected =
    revisionHistoryState.items[revisionHistoryState.selectedIndex];

  if (!activeTab || !selected) {
    showStatus(t("msg-history-empty"));
    return;
  }
  ...
  activeTab.content = selected.content || "";
  activeTab.isModified = true;
  loadActiveTabContent();
  ...
  if (currentTabUI) currentTabUI.classList.add("modified");

  await saveToStorage();
  await openRevisionHistoryPanel();
  showStatus(t("msg-history-restored"));
}
...
async function deleteSelectedRevision() {
  ...
  const snapshots = await getSnapshotsByTabId(activeTab.id);
  snapshots.splice(selected.sourceIndex, 1);
  await setSnapshotsByTabId(activeTab.id, snapshots);

  revisionHistoryState.selectedIndex = Math.min(
    revisionHistoryState.selectedIndex,
    snapshots.length - 1,
  );
  await openRevisionHistoryPanel();
  showStatus(t("msg-history-deleted"));
}

function toggleRevisionDiffView() {
  revisionHistoryState.viewMode =
    revisionHistoryState.viewMode === "diff" ? "preview" : "diff";
  renderRevisionHistoryPanel();
}

function toggleChangedOnlyFilter() {
  if (revisionHistoryState.viewMode !== "diff") return;
  revisionHistoryState.showChangedOnly = !revisionHistoryState.showChangedOnly;
  renderRevisionHistoryPanel();
}
...
async function updateMarkdownPreview() {
  ...
  if (appData.markdownMode) {
    ...
    if (docText.length > 100000) {
      preview.textContent =
        "⚠️ 문서가 너무 큽니다 (100KB 초과). 마크다운 프리뷰가 비활성화되었습니다.";
    ...
    const codeBlockPhs = [];
    let codeProtected = docText.replace(
      /^[ ]{0,3}```[\s\S]*?^[ ]{0,3}```/gm,
      (match) => {
    ...
    const katexBlocks = [];
    const katexExtracted = codeProtected.replace(
      /\$\$([\s\S]*?)\$\$/gm,
      (match, formula) => {
    ...
    const processedText = katexProtected.replace(
      /^[ ]{0,3}```mermaid\s*\n([\s\S]*?)^[ ]{0,3}```/gm,
      (match, code, offset) => {
            ...
            if (cjkPattern.test(processedCode)) {
              ...
              processedCode = processedCode
                ...
                .map((line) => {
                  ...
                  if (
                    isXYChart &&
                    /^\s*x-axis\s+\[/.test(trimmed) &&
                    cjkPattern.test(trimmed)
                  ) {
                    return line.replace(/\[([^\]]+)\]/, (m, inner) => {
                      const items = inner.split(",").map((s) => {
                        const v = s.trim();
                        return v.startsWith('"') || !cjkPattern.test(v)
                          ? v
                          : `"${v}"`;
                      });
                      return `[${items.join(", ")}]`;
...
function showStatus(msg) {
  ...
  statusTimeout = setTimeout(() => {
    const readyMsg =
      (i18nDict[appData.uiLang] && i18nDict[appData.uiLang]["status-ready"]) ||
      "준비됨";
...
async function handleOpenFile() {
  try {
    if (window.showOpenFilePicker) {
      ...
      input.type = "file";
      input.accept =
        ".txt,.md,.json,.js,.html,.css,.py,.c,.cpp,.rs,.cs,.java,.sql";
      input.onchange = (e) => {
  ...
  try {
    ...
    if (suggestedName.includes("무제") || suggestedName.includes("새 문서")) {
      const newName = prompt(
        t("prompt-save-name", "Enter file name (syntax coloring will apply):"),
        "untitled.txt",
      );
      if (newName && newName.trim() !== "") {
    ...
    if (window.showSaveFilePicker) {
      ...
      if (!handle || saveAs) {
        let suggestedName = activeTab.title;
        if (
          suggestedName.includes("무제") ||
          suggestedName.includes("새 문서")
        ) {
...
function setupRevisionHistoryController() {
  const historyModal = document.getElementById("history-modal");
  const historyList = document.getElementById("history-list");
  const compareButton = document.getElementById("btn-history-compare");
  const filterButton = document.getElementById("btn-history-filter");
  const deleteButton = document.getElementById("btn-history-delete");
  const restoreButton = document.getElementById("btn-history-restore");
  const closeButton = document.getElementById("btn-history-close");
  ...
  if (historyList) {
    historyList.addEventListener("click", (e) => {
      ...
      if (!item) return;
      revisionHistoryState.selectedIndex = parseInt(item.dataset.index, 10);
      renderRevisionHistoryPanel();
    });
  }
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", () => setFontSize(appData.fontSize + 2));
  document
  ...
  langSelect.addEventListener("change", (e) => {
    ...
    if (activeTab) {
      ...
      const langDesc = languages.find(
        (l) =>
          l.name === activeTab.lang ||
          l.name.toLowerCase() === activeTab.lang.toLowerCase() ||
          (l.alias && l.alias.includes(activeTab.lang.toLowerCase())),
      );
      if (langDesc) {
        langDesc
          .load()
  ...
  if (toggleVim) {
    ...
    toggleVim.addEventListener("change", (e) => {
      ...
      cm.dispatch({
        effects: vimConf.reconfigure(appData.vimMode ? vim() : []),
      });
      saveToStorage();
  ...
  document
    ...
    .addEventListener("click", () => handleSaveFile(false));
  document
    .getElementById("btn-save-as")
    .addEventListener("click", () => handleSaveFile(true));
  document.getElementById("btn-open").addEventListener("click", handleOpenFile);

  // Backup & Restore
  document
    .getElementById("btn-backup")
    .addEventListener("click", backupWorkspace);
  document
  ...
  document.getElementById("btn-cut").addEventListener("click", () => {
    ...
    if (activeTab && activeTab.readonly) {
      showStatus(t("msg-readonly-locked"));
      return;
    }
    ...
    if (selection) {
      navigator.clipboard
        ...
        .then(() => {
          ...
          showStatus(
            i18nDict[appData.uiLang]["status-ready"] || "잘라내기 완료",
          );
        })
  ...
  document.getElementById("btn-copy").addEventListener("click", () => {
    ...
    if (selection) {
      navigator.clipboard
        .writeText(selection)
        .then(() =>
          showStatus(i18nDict[appData.uiLang]["status-ready"] || "복사 완료"),
        )
        .catch((e) => showStatus(t("msg-clipboard-denied")));
    }
  });
    ...
    navigator.clipboard
      ...
      .then((text) => {
        if (text) {
          ...
          showStatus(
            i18nDict[appData.uiLang]["status-ready"] || "붙여넣기 완료",
          );
        }
      })
      .catch((e) => showStatus(t("msg-clipboard-denied")));
  ...
  btnReadonly.addEventListener("click", () => {
    ...
    if (activeTab) {
      ...
      saveToStorage();
      showStatus(
        activeTab.readonly ? t("msg-readonly-locked") : t("status-ready"),
      );
    }
  });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (modKey && e.shiftKey && e.key.toLowerCase() === "t") {
      ...
      e.preventDefault();
      const currentIdx = appData.tabs.findIndex(
        (t) => t.id === appData.activeTabId,
      );
      if (appData.tabs.length > 1) {
        let nextIdx;
        if (e.key === "PageUp") {
          // Ctrl+PageUp → 이전 탭 (순환)
          nextIdx =
            (currentIdx - 1 + appData.tabs.length) % appData.tabs.length;
        } else {
  ...
  document
    .getElementById("btn-confirm-discard")
...
function setupSearchController() {
  ...
  function doSearch(reverse = false) {
    ...
    if (
      query !== window.searchLastQuery ||
      window.searchAllMatches.length === 0
    ) {
      window.searchAllMatches = [];
      let cur = new SearchCursor(cm.state.doc, query);
      while (!cur.next().done) {
        window.searchAllMatches.push({
          from: cur.value.from,
          to: cur.value.to,
        });
      }
      ...
      if (window.searchAllMatches.length === 0) {
        searchStatus.textContent =
          i18nDict[appData.uiLang]["status-error"] || "결과 없음";
        return;
      }
    }
  ...
  document
    ...
    .addEventListener("click", () => doSearch(false));
  document
    .getElementById("btn-search-prev")
    .addEventListener("click", () => doSearch(true));
  ...
  document.getElementById("btn-replace").addEventListener("click", () => {
    ...
    if (selectedText === query) {
      cm.dispatch({
        ...
        selection: {
          anchor: mainSel.from,
          head: mainSel.from + replacement.length,
        },
        scrollIntoView: true,
      });
      searchStatus.textContent =
        i18nDict[appData.uiLang]["status-ready"] || "변경됨";
  ...
  document.getElementById("btn-replace-all").addEventListener("click", () => {
    ...
    if (window.searchAllMatches.length > 0) {
      let changes = window.searchAllMatches.map((m) => ({
        from: m.from,
        to: m.to,
        insert: replacement,
      }));
...
async function initApp() {
  ...
  if (typeof mermaid !== "undefined") {
    const isDark = ["dark", "monokai", "solarized-dark"].includes(
      appData.theme,
    );
    mermaid.initialize({
      startOnLoad: false,
...
async function bootstrap() {
  ...
  setupRevisionHistoryController();
}
